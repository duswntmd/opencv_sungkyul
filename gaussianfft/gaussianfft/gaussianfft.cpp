...
int main(int argc, char* argv[])
{
	...
	Gaussian = (double**)d_alloc(COL, ROW);
	...
	if (Mode == 0) {
		...
		fft_2d(fourier_img, imaginary_img, ROW, Mode);
		//strcat(name, "cmangaussianfft1");
		//strcat(dname, "cmangaussianfft1");
		strcat(name, "lenahighgaussianfft2");
		strcat(dname, "lenahighgaussianfft2");
		...
		for (i = 0; i < ROW; i++)
			for (j = 0; j < COL; j++)
			{
				...
				//Gaussian[i][j] = exp(-(diameter * diameter) / (2 * D0 * D0)); //low
				Gaussian[i][j] = 1 - exp(-(diameter * diameter) / (2 * D0 * D0)); //high

				fourier_img[i][j] *= Gaussian[i][j];
				imaginary_img[i][j] *= -Gaussian[i][j];
			}
